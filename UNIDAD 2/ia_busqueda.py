...
def main():
    ...
    nombre_archivo = "UNIDAD 2/registro.txt"
    ...
    movimientos = len(nodos_camino)
    
    #Registra hora y fecha, estado inicial, coordenadas a rastrear, numero de movimientos, movimientos realizados
    with open(nombre_archivo, 'a') as archivo:
        # Escribe en el archivo
        archivo.write(f"--------------------------------------------------------------------------------------------------\n")
        archivo.write(f"{fecha_hora_actual} - Estado inicial : {estado_inicial}\n")
        archivo.write(f"{fecha_hora_actual} - Numero a rastrear: {n} - Coordenadas iniciales: {x},{y} - Coordenadas finales: {xf},{yf}\n")
        
        if movimientos is None:
            archivo.write(f"{fecha_hora_actual} - El puzzle no se pudo resolver\n")
        else:
            archivo.write(f"{fecha_hora_actual} - Numero de movimientos realizados para resolver: {movimientos}\n")
        for nodo in nodos_camino:
               m = nodo.movimiento
               archivo.write(f"{fecha_hora_actual} - Movimiento : {m} \n" )
               
        archivo.write(f"{fecha_hora_actual} - Estado final : {estado_final}\n")
